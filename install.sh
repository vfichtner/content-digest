...
echo "         client = TelegramClient("
echo "           os.path.expanduser('~/.claude/config/telegram.session'),"
echo "           int(os.environ['TELEGRAM_API_ID']),"
echo "           os.environ['TELEGRAM_API_HASH']"
echo "         )"
echo "         async def auth():"
echo "           await client.start()"
echo "           print('Authenticated!')"
echo "           await client.disconnect()"
echo "         asyncio.run(auth())"
echo "       \""
echo
